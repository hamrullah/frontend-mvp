...
const RESOURCE = "Login"; // sesuaikan dengan controller kamu
...
export default function SalesAnalyticsPage() {
  ...
  // Date range default: 30 hari terakhir (Asia/Jakarta friendly)
  ...
  // Auth header (tanpa Bearer sesuai backend kamu)
  ...
  const fetchOrders = async () => {
    ...
    setError("");
    try {
      // Ambil banyak sekalian lalu filter di client (karena API listOrder belum ada filter tanggal)
      const { data } = await axios.post(
        `${API_BASE}/${RESOURCE}/listOrder`,
        { limit: 5000, offset: 0 },
        { headers: authHeaders }
      );
    ...
    <section className="section">
      <div className="container">
                  ...
                  <table className="table is-fullwidth is-striped is-hoverable is-narrow">
                    ...
                    {dailySeries.length > 0 && (
                      <tfoot>
                        <tr>
                          <th>Total</th>
                          <th className="has-text-right">{dailySeries.reduce((a, b) => a + b.count, 0)}</th>
                          <th className="has-text-right">{toRupiah(dailySeries.reduce((a, b) => a + b.omzet, 0))}</th>
                        </tr>
                      </tfoot>
                    )}
                  </table>
        ...
        <p className="has-text-grey is-size-7" style={{ marginTop: 12 }}>
          Sumber data: <code>POST /{RESOURCE}/listOrder</code> (client-side filter by date)
        </p>
      </div>
    </section>
  );
